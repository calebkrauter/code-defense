...
public class CodeDefense {
    ...
    static private String promptForName(final String firstOrLast, final Scanner input) {
        ...
        if (mismatchedInput(name, "^[a-zA-Z]{1,50}$")) {
            System.out.println("Not a valid name. Please try again.");
            logToFile("Invalid name: " + name);
    ...
    static private BigInteger promptForInt(final Scanner input) {
        ...
        try {
            ...
            if (min.compareTo(n) > 0 || max.compareTo(n) < 0) {
                ...
                logToFile("Integer out of range: " + n.toString());
                return promptForInt(input);
            }

        } catch(Exception e) {
            logToFile("Failed to get integer: " + e);
            return promptForInt(input);
        }
    ...
    static private String promptForInputFileName(final Scanner input) {
        ...
        if (mismatchedInput(name, "^[a-zA-Z0-9]{1,50}.txt$")) {
            ...
            logToFile("INPUT FILE NAME MISMATCH: " + name + " doesn't match regex pattern: " + "^[a-zA-Z0-9]{1,50}.txt$");
    ...
    static private String promptForOutputFileName(final Scanner input, final String inputFileName) {
        ...
        if (mismatchedInput(name, "^[a-zA-Z0-9]{1,50}.txt$") || name.equals(inputFileName)) {
            System.out.println("Not a valid file name. Please try again.");
            logToFile("OUTPUT FILE NAME MISMATCH: " + name + " doesn't match regex pattern: " + "^[a-zA-Z0-9]{1,50}.txt$");
    ...
    static private String promptForPassword(final Scanner input, final String message)
        ...
        if (mismatchedInput(password, "^(?=.*[A-Z])(?=.*[a-z])(?=.*\\d)(?=.*[\\.,!\\?'\";:-])"
            ...
            logToFile("PASSWORD MISMATCH, PASSWORD: " + password + " doesn't match regex pattern: " + "^(?=.*[A-Z])(?=.*[a-z])(?=.*\\d)(?=.*[\\.,!\\?'\";:-])");
     ...
     * @throws IOException
     */
    static private void storePassword(final Scanner input) throws InvalidKeySpecException, NoSuchAlgorithmException, IOException {
        ...
        try {
            ...
            logToFile("COULD NOT STORE PASSWORD, EXCEPTION: " + e + " password used: " + password);
            storePassword(input);
        }
    }

    static private void getPasswords(final Scanner input) throws InvalidKeySpecException, NoSuchAlgorithmException, IOException {
        storePassword(input);
     ...
     */
    static private void comparePassword(final Scanner input) throws InvalidKeySpecException, NoSuchAlgorithmException, IOException {
        ...
        try (Scanner file = new Scanner(passwordFile)) {
            ...
            logToFile("PASSWORD FILE DNE " + passwordFile.toString());
            getPasswords(input);
            return;
        }
        ...
        if (!Arrays.equals(hash, hash2)) {
            ...
            String h1 = new String(hash);
            String h2 = new String(hash2);
            logToFile("PASSWORDS DO NOT MATCH, PASSWORD 1 DOES" + h1 + " != " + h2);
            comparePassword(input);
        } else {
    ...
    static private void writeToFile(final String firstName, final String lastName, final BigInteger int1,
        ...
        try (Scanner fileScanner = new Scanner(inputFile))  {
            ...
            logToFile("FILE NOT FOUND with inputFile =" + inputFile.toString());
    ...
    static private void logToFile(String log) {
        String logFileName = "resources/log.txt";
        PrintWriter output = null;
        try {
            output = new PrintWriter(new FileWriter(logFileName, true));
        } catch (IOException e) {
            return;
        }
            
        LocalDateTime curTime = LocalDateTime.now();
        output.println("Logged bad input at " + curTime + " Error occured, logged issue: " + log + "\n");
        output.close();
